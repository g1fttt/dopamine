...
impl Renderer {
  ...
  #[inline]
  pub fn textures_mut(&mut self) -> &mut Textures<IDirect3DTexture9> {
    &mut self.textures
  }

  /// The textures registry of this renderer.
  #[inline]
  pub fn textures(&self) -> &Textures<IDirect3DTexture9> {
    &self.textures
  }
  ...
  unsafe fn write_buffers(&mut self, draw_data: &DrawData) -> WindowsResult<()> {
    let (mut vtx_dst, mut idx_dst) = unsafe {
      Self::lock_buffers(
  ...
  unsafe fn create_font_texture(
    ...
    unsafe { result_texture.UnlockRect(0)? };
