...
void Hooks::setup(core::Interfaces &interfaces, core::Patterns &patterns,
                  HWND window) {
  this->window = window;
  ...
  const auto engine = interfaces.engine;
  ...
  d3d9_reset_original =
      **patterns.d3d9_reset.cast<decltype(d3d9_reset_original)>();

  **patterns.d3d9_present.cast<decltype(d3d9::present) **>() = d3d9::present;
  **patterns.d3d9_reset.cast<decltype(d3d9::reset) **>() = d3d9::reset;
}

void Hooks::remove(core::Patterns &patterns) {
  **patterns.d3d9_present.cast<decltype(d3d9::present) **>() =
      d3d9_present_original;
